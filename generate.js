...
- INTERDIT de copier le template : "Ce que fait ce composant en une phrase"`;
  ...
  if (!json.description || json.description.includes("Ce que fait ce composant")) {
...
async function main() {
  ...
  console.log(existingGuide ? "✔ Guide existant chargé" : "  Première génération\n");
  ...
  // 3 appels séparés
  const [description, etapes, erreurs] = await Promise.all([
    generateDescription(fullCode, componentName, existingGuide?.sections?.description),
    generateEtapes(fullCode, componentName, existingGuide?.sections?.etapes),
    generateErreurs(fullCode, componentName, existingGuide?.sections?.erreurs),
  ]);
